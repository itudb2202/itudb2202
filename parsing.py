...
#       ----------KICKOFF---------
data = pd.read_csv("Career_Stats_Kickoff.csv")
column_names = ["Player Id", "Year", "Team", "Games Played", "Kickoffs", "Kickoff Yards",
                "Out of Bounds Kickoffs","Yards Per Kickoff","Touchbacks"]
for data_column in data.columns:
    if data_column not in column_names:
        data.drop(data_column, inplace = True, axis=1)
for inr in data.index:
    ...
    for column_name in column_names:
        if (column_index != len(column_names) - 1):
            ...
            insertValues = insertValues + str(row[column_name])
        column_index = column_index + 1
    statement = "INSERT INTO career_stats_kickoff VALUES (%s)" % insertValues
    

# ---------- PASSSING ---------
data = pd.read_csv("Career_Stats_Passing.csv")
column_names = ["Player Id", "Year", "Team", "Games Played", "Passes Attempted", "Passes Completed",
...
for inr in data.index:
    ...
    for column_name in column_names:
        if (column_index != len(column_names) - 1):
            ...
            insertValues = insertValues + str(row[column_name])
        column_index = column_index + 1
    statement = "INSERT INTO career_stats_passing VALUES (%s)" % insertValues
